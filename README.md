...
如果 TTS 返回 `504 Gateway Time-out`，优先检查参考音频是否太短、是否和 `reference_text` 内容不匹配。朗读示例文本模式会把整段示例文本作为 `reference_text`，需要完整朗读；如果只说几秒钟，建议切换到 `自由说话` 模式。
...
- `录音`：浏览器申请麦克风权限，录制完成后作为参考音频上传。
- `上传音频`：保留本地音频文件上传入口。

说话模式有两种：

- `朗读示例文本`：页面展示固定示例文本，并自动把这段文本作为 `reference_text` 传给后端。录音少于 15 秒时会被前端拦截，建议录制 20-40 秒。
